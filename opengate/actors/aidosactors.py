...
from opengate.logger import logger
...
class AIDosActor(ActorBase, g4.GateAIDosActor):  # type: ignore
    user_info_defaults = {
        ...
        "update_histograms_threshold": (
            10,
            {"doc": "Hits per voxel before variance update"},
        ),
        ...
        "rel_error_percentile": (
            0.95,
            {"doc": "Percentile of voxels to satisfy threshold"},
        ),
        "convergence_region_mode": (
            "all",
            {"doc": "all | no_enclosure | no_objects | no_enclosure_no_objects"},
        ),
        "tracer_type": ("DDA", {"doc": "DDA only"}),
        "scoring_quantities": (
            ["transported_energy", "ambient_dose"],
            {"doc": "transported_energy, energy_fluence, air_kerma, ambient_dose"},
        ),
        "adeph_path": (
            "",
            {"doc": "Path to adeph.dat (ambient dose conversion factors)"},
        ),
        "mu_en_air_path": (
            "",
            {"doc": "Path to mu_en_air.dat (NIST air mass-energy coefficients)"},
        ),
        ...
        "output_path": (
            ".",
            {"doc": "Output directory (overridden by simulation.output_dir)"},
        ),
    ...
    def initialize(self) -> None:
        ...
        self.user_info["world_size"] = list(
    ...
    def EndSimulationAction(self) -> None:
        logger.info(
            f"[AIDosActor] Simulated primaries: {g4.GateAIDosActor.GetNumberOfAbsorbedEvents(self)}"
        )
